...
const schema = yup.object().shape({
	analogOptions: yup.object().shape({
		enabled: yup.number().label('Enabled'),
	}),
	boardLedOptions: yup.object().shape({
		enabled: yup.number().label('Enabled'),
		dataPin: yup.number().label('Data Pin'),
		ledFormat: yup.number().label('LED Format'),
		ledLayout: yup.number().label('LED Layout'),
		ledsPerButton: yup.number().label('LEDs Per Button'),
		brightnessMaximum: yup.number().label('Max Brightness'),
		brightnessSteps: yup.number().label('Brightness Steps'),
	}),
	bootselButtonOptions: yup.object().shape({
		enabled: yup.number().label('Enabled'),
		buttonMask: yup.number().label('Button Mask'),
	}),
	buzzerSpeakerOptions: yup.object().shape({
		enabled: yup.number().label('Enabled'),
		buzzerPin: yup.number().label('Buzzer Pin'),
		buzzerVolume: yup.number().label('Buzzer Volume'),
	}),
	dualInputOptions: yup.object().shape({
		enabled: yup.number().label('Enabled'),
	}),
	extraButtonOptions: yup.object().shape({
		enabled: yup.number().label('Enabled'),
	}),
	i2cAnalog1219Options: yup.object().shape({
		enabled: yup.number().label('Enabled'),
		i2cAddress: yup.number().label('I2C Address'),
		i2cBlock: yup.number().label('I2C Block'),
	}),
	displayOptions: yup.object().shape({
		enabled: yup.number().label('Enabled'),
		i2cAddress: yup.number().label('I2C Address'),
		i2cBlock: yup.number().label('I2C Block'),
		i2cSpeed: yup.number().label('I2C Speed'),
		buttonLayout: yup.number().label('Button Layout'),
		buttonLayoutRight: yup.number().label('Button Layout Right'),
		splashMode: yup.number().label('Splash Mode'),
		splashDuration: yup.number().label('Splash Duration'),
		displaySaverTimeout: yup.number().label('Display Saver Timeout'),
		invertDisplay: yup.number().label('Invert Display'),
		flipDisplay: yup.number().label('Flip Display'),
	}),
	keyboardHostOptions: yup.object().shape({
		enabled: yup.number().label('Enabled'),
		pinDplus: yup.number().label('Pin D+'),
		pinDminus: yup.number().label('Pin D-'),
		pin5V: yup.number().label('Pin 5V'),
		mouseMovement: yup.number().label('Mouse Movement'),
	}),
	playerNumberOptions: yup.object().shape({
		enabled: yup.number().label('Enabled'),
		number: yup.number().label('Number'),
	}),
	ps4ModeOptions: yup.object().shape({
		enabled: yup.number().label('Enabled'),
	}),
	reverseOptions: yup.object().shape({
		enabled: yup.number().label('Enabled'),
		buttonPin: yup.number().label('Button Pin'),
		ledPin: yup.number().label('LED Pin'),
		action: yup.number().label('Action'),
	}),
	sliderOptions: yup.object().shape({
		enabled: yup.number().label('Enabled'),
		pinLS: yup.number().label('LS Pin'),
		pinRS: yup.number().label('RS Pin'),
	}),
	snesInputOptions: yup.object().shape({
		enabled: yup.number().label('Enabled'),
		clockPin: yup.number().label('Clock Pin'),
		latchPin: yup.number().label('Latch Pin'),
		dataPin: yup.number().label('Data Pin'),
	}),
	socdSliderOptions: yup.object().shape({
		enabled: yup.number().label('Enabled'),
	}),
	tiltOptions: yup.object().shape({
		enabled: yup.number().label('Enabled'),
		pin1: yup.number().label('Pin 1'),
		pin2: yup.number().label('Pin 2'),
		pin3: yup.number().label('Pin 3'),
		pin4: yup.number().label('Pin 4'),
	}),
	touchpadDataOptions: yup.object().shape({
		enabled: yup.number().label('Enabled'),
	}),
	turboOptions: yup.object().shape({
		enabled: yup.number().label('Enabled'),
		buttonPin: yup.number().label('Button Pin'),
		ledPin: yup.number().label('LED Pin'),
		shotCount: yup.number().label('Shot Count'),
		shmupMode: yup.number().label('Shmup Mode'),
		shmupMixMode: yup.number().label('Shmup Mix Mode'),
		shmupAlwaysOn1: yup.number().label('Shmup Always On 1'),
		shmupAlwaysOn2: yup.number().label('Shmup Always On 2'),
		shmupAlwaysOn3: yup.number().label('Shmup Always On 3'),
		shmupAlwaysOn4: yup.number().label('Shmup Always On 4'),
		shmupBtn1Pin: yup.number().label('Shmup Button 1 Pin'),
		shmupBtn2Pin: yup.number().label('Shmup Button 2 Pin'),
		shmupBtn3Pin: yup.number().label('Shmup Button 3 Pin'),
		shmupBtn4Pin: yup.number().label('Shmup Button 4 Pin'),
		shmupBtnMask1: yup.number().label('Shmup Button 1 Mask'),
		shmupBtnMask2: yup.number().label('Shmup Button 2 Mask'),
		shmupBtnMask3: yup.number().label('Shmup Button 3 Mask'),
		shmupBtnMask4: yup.number().label('Shmup Button 4 Mask'),
	}),
	wiiextOptions: yup.object().shape({
		enabled: yup.number().label('Enabled'),
		i2cBlock: yup.number().label('I2C Block'),
	}),
	// 追加：JinglePlayerのバリデーション
