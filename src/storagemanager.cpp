...
{
	// =================================================================
	// 🚀 MINI Super - Live Memory Snapshot Backup Logic (現行メモリ即時直流し込み)
	// =================================================================
	
	// 1. EEPROMクラスを一度初期化して書き込みバッファをクリア
	EEPROM.reset();

	// 2. 💡【超重要】今まさにPICOのメモリ上で完璧に動いている「this->config」の実体をそのまま使用。
	// WebConfigの通信バグで破損したJSON等は一切経由せず、現在の完璧な構造体のスナップショットを
	// そのままシステム標準のシリアライザに渡して、32KBの物理バッファ（writeCache）へ直流し込みバイナリ化します。
	ConfigUtils::save(this->config);

	// 3. 自動合成された完璧な生バイナリを、そのまま物理フラッシュメモリへ本物のセーブデータとして強制永続化
	EEPROM.commit();

	// 4. 保存が完全に完了した状態で、ウォッチドッグで安全にクールドリブート（再起動）
