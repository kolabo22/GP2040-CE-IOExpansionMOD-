...
bool TurboInput::available() {
	...
	if (!options.isConfigured) {
		...
		pinMappings[14].action = GpioAction::BUTTON_PRESS_TURBO;
	}

	// Turbo Button initialized by void Gamepad::setup()
	hasTurboAssigned = false;
	turboPinMask = 0; // マスクの初期化を保証
	...
	return options.enabled && (hasTurboAssigned == true);
}
...
void TurboInput::setup(){
	...
	if ( options.shmupModeEnabled ) {
		...
		alwaysEnabled = options.shmupAlwaysOn1 | options.shmupAlwaysOn2 | options.shmupAlwaysOn3 | options.shmupAlwaysOn4;
