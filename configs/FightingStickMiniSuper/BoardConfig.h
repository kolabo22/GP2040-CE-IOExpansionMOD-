...
// レバー4方向（初期設定のままネイティブロードで100%大復活！）
...
// メイン30φボタン (8ボタン ＆ 初期設定のままネイティブロードで100%大復活！)
...
// 機能ボタン (TURBO, S2/STARTは完全に独立した通常ボタンとして固定。ホットキー移行に委ねます)
...
// コア通信ブロックの強制有効化（11〜12ページ目の公式マクロ名に完全適合）
...
// ====================================================================
// 3. 各種アドオン機能の初期動作設定（公式システムマクロ名に100%完全同期）
// ====================================================================
// Wii拡張アドオン（19ページ目の公式変数「WII_EXTENSION_ENABLED」に適合）
#define WII_EXTENSION_ENABLED 1
#define WII_EXTENSION_I2C_BLOCK i2c0
#define WII_EXTENSION_I2C_SPEED 400000

// 連射アドオン（16ページ目の公式変数「PIN_SHMUP_DIAL」へ名前を完全同期：ボタン14、LED15、VR26速度制御）
...
// リアクティブLEDアドオン（21ページ目の本物のマクロ名配列定義：ピンとリバースフェードの空欄化を完全阻止）
#define REACTIVE_LED_ENABLED 1
#define REACTIVE_LED_COUNT 4
#define REACTIVE_LED_PINS { 16, 22, 23, 24 }
#define REACTIVE_LED_MODES_DOWN { 2, 2, 2, 2 } // 2 = STATIC_OFF（押すと消える）
#define REACTIVE_LED_MODES_UP { 1, 1, 1, 1 }   // 1 = STATIC_ON（離すと光る）

// ====================================================================
// 4. LED構成・点灯順序・変則ケースLED（13ページ目の公式仕様に完全同期）
// ====================================================================
...
#define DISPLAY_SAVER_MODE static_cast<DisplaySaverMode>(2) // 2 = 雪モード固定
...
// ====================================================================
// 6. PCF8575 IO エクスパンダー 16ピン入力キーマッピング（20ページ完全同期）
// ====================================================================
...
// ====================================================================
// 7. 【エラー完封・記憶保護マクロ】他ファイルを絶対汚さない安全な置換定義
// ====================================================================
// 1471行目の未定義エラーを完封。かつ、処理を一切持たないため「多重定義エラー（全ファイル即死）」や
// 「物理ピンロード処理の強制上書きスキップ（文鎮化）」を100%根本から回避します。
#define gpioMappingsMigrationCore(config)
